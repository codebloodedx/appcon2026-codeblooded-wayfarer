...
backend/src/                      Ranee: Groq/Qwen classification, rule lookup and grounded NLP API
...
Browser camera (live) → Bryan's frame callback → Ranee's guidance adapter
  → backend Groq/Qwen sign classification → strict supported-ID validation
  → John's source-reviewed rule record → brief UI alert and browser speech synthesis
...
- Groq/Qwen identifies a candidate sign; the reviewed record supplies the traffic rule. Validate sign IDs and country before alerting.
- Keep `GROQ_API_KEY` on the backend. The browser Maps key is restricted by origin. Never commit either key.
