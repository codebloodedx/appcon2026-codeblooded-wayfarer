...
| --- | --- | --- | --- | --- |
| **Ranee Mikaella Gutierrez** | `@seavens3nt` | Project setup, contracts, Gemini/Vertex recognition and rule-grounded NLP, browser voice, Maps, country detection, route guidance and labeled restriction/proximity simulations, final integration/review/submission | root setup files; `backend/**`; `frontend/src/features/map/**`; `frontend/src/features/guidance/**`; project governance/context docs | Specific proficiency and available hours **not verified**; workload is the largest capacity risk. |
| **Gio Joshua Gonzales** | `@GiYo-Mi02` | Complete traveler-facing UI: trip setup, active trip and route summary, restricted-zone control, Reviewed Guidance, supported signs, high-contrast two-wheeler/device previews, responsive/error/accessibility states | `frontend/src/App.tsx`; `frontend/src/features/trip/**`; `frontend/src/components/**`; `frontend/src/styles.css` | Specific proficiency and available hours **not verified**. |
...
- Bryan can implement the camera subsystem against documented callback props, without calling Vertex AI or editing Gio's UI.
